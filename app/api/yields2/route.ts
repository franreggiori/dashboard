  ...
  try {
    ...
    for (const { ticker, settlement = "A-24HS" } of tickers) {
      ...
      try {
        ...
        const tirLast = (lastPriceUSD ?? 0) > 0 ? await getTir(token, ticker, lastPriceUSD!) : null;
        await sleep(SLEEP_TICKERS_MS);
        const yBid    = (bidPxUSD     ?? 0) > 0 ? await getTir(token, ticker, bidPxUSD!)     : null;
        await sleep(SLEEP_TICKERS_MS);
        const yAsk    = (askPxUSD     ?? 0) > 0 ? await getTir(token, ticker, askPxUSD!)     : null;
